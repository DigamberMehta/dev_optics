...
export default function NewArrivals({ products }) {
  ...
  return (
    <motion.div
      className="bg-blue-50 p-3 md:p-8 rounded-lg relative m-3"
      ref={containerRef}
    ...
    >
      ...
      <div className="relative">
        ...
        <button
          className="absolute left-40 top-1/2 -translate-y-1/2 bg-white p-3 rounded-full shadow-md z-20 hover:bg-gray-200 transition hidden md:block"
          onClick={() => scroll("left")}
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <div className="flex gap-6 overflow-x-scroll scroll-smooth no-scrollbar px-3 hide-scrollbar">
          <button className="text-[#4ebac6] text-3xl font-medium hover:underline whitespace-nowrap hidden md:block">
            View All
          </button>
          <div
            ref={scrollRef}
            className="flex gap-6 overflow-x-scroll scroll-smooth no-scrollbar p-0 md:px-16 hide-scrollbar"
          >
            {newArrivalProducts.map((product) => (
            <motion.div
            key={product.product_id}
            className="product-card bg-white rounded-lg p-2 md:p-4 w-[180px] h-[230px] max-w-[195px] md:min-w-[190px] md:max-w-[210px] flex-shrink-0 cursor-pointer transition h-fit md:h-[250px]"
          ...
          >
            <img
              ...
              alt={product.name}
              className="w-full rounded-lg h-[110px] object-cover"
            />
            <div className="mt-2">
        ...
        <button
          className="bg-white p-3 rounded-full shadow-md absolute right-2 top-1/2 -translate-y-1/2 z-20 hover:bg-gray-200 transition hidden md:block"
